...
def generate_launch_description():
    ...
    nav2_launch_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'autostart': autostart,
            'map': os.path.join(pkg_mcl, 'map', 'bcr_map.yaml'),
            'params_file': os.path.join(pkg_mcl, 'config', 'nav2_params.yaml'),
            'package_path': pkg_mcl, 
        }.items()
    )
    ...
    map_server_node = Node(
        ...
        name='map_server',
        output='screen',
        parameters=[{'yaml_filename': os.path.join(pkg_mcl, 'map', 'bcr_map.yaml')}],
    )
